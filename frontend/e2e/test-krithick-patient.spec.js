...
test('Test Returning Patient (KRITHICK THANGARAJ) Registration & Queue Display', async ({ page }) => {
  // 1. Open Live Login
  ...
  // 4. Search for existing patient "KRITHICK"
  const searchInput = page.locator('input[placeholder*="Scan Barcode"]').first();
  await searchInput.fill('6382003360');
  
  // Wait for search result dropdown item and click it
  const dropdownItem = page.locator('.search-results-list li, .patient-search-results li, ul li:has-text("KRITHICK")').first();
  ...
  await expect(page.locator('text=KRITHICK').first()).toBeVisible({ timeout: 10000 });

  // 9. Verify "KRITHICK THANGARAJ" is in the live queue list
  const queueSection = page.locator('.registration-sidebar, aside');
  await expect(queueSection.locator('text=KRITHICK').first()).toBeVisible({ timeout: 10000 });
});

...
export default function Consultation() {
  ...
  const handleSave = async (statusToSet = 'completed') => {
    ...
    setSaving(true);
    try {
      // Validate minimum required data for completion
      if (statusToSet === 'completed') {
        if (visitComplaints.length === 0) {
          alert('Please add at least one chief complaint before completing.');
          setSaving(false);
          return;
        }
        if (visitDiagnosis.length === 0) {

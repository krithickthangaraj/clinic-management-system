...
export default function Consultation() {
      ...
      try {
        ...
        try {
          ...
          if (mounted && pData) {
            ...
            if (pData.medicines && pData.medicines.length > 0) setMedicines(pData.medicines);
            if (pData.plan_and_billing) setPlanAndBilling((prev) => ({ ...prev, ...pData.plan_and_billing }));
          }
        } catch {
        ...
        setErrorMessage('Failed to load consultation data. Please check connection.');
      } finally {
  ...
  const handleAction = async (actionType) => {
    try {
      ...
      // Validation: at least one valid drug
      const validMedicines = medicines.filter((m) => m.drug_name && m.drug_name.trim());
      ...
      setErrorMessage(
        err.response?.data?.detail || 'Failed to save prescription. Please try again.'
      );
    } finally {
      setSaving(false);
    }
  ...
  if (loading) {
    return (
      <div className="rx-consultation-page flex items-center justify-center p-12">
        <div className="flex flex-col items-center gap-3">
          <div className="w-8 h-8 border-3 border-teal-600 border-t-transparent rounded-full animate-spin" />
          <span className="text-sm font-semibold text-slate-600">Loading Consultation Desk...</span>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="rx-consultation-page">
      ...
      {/* Main Form Content */}
      <div className="rx-consultation-container">
        {/* Toast / Error alerts */}
        {successToast && (
          <div className="p-3 bg-emerald-50 border border-emerald-200 text-emerald-800 rounded-lg text-sm font-semibold flex items-center justify-between shadow-sm">
            <span>✓ {successToast}</span>
          </div>
        )}

        {errorMessage && (
          <div className="p-3 bg-rose-50 border border-rose-200 text-rose-800 rounded-lg text-sm font-semibold flex items-center justify-between shadow-sm">
        ...
        {/* 1. Top Ribbon: Patient Demographics, Timing & Vitals */}
        <PatientVitalsHeader
          patient={patient}
          visit={visit}
          vitals={vitals}
        ...
        {/* 2. Collapsible Medical History Sidebar / Accordion */}
        ...
        {/* 3. Clinical Assessment Form (Complaints, Diagnosis, Examination) */}
        <ClinicalAssessmentForm
          assessment={assessment}
          onChange={setAssessment}
        />

        {/* 4. Core Engine: RX Medication Spreadsheet Table */}
        <RXMedicationTable
          medicines={medicines}
          onAddDrug={addDrug}
      ...
      {/* 5. Sticky Bottom Bar: Plan, Billing & Action Buttons */}
      <PrescriptionFooter

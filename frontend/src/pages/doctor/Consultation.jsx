...
const DOSAGE_OPTIONS = ['250mg', '500mg', '750mg', '1000mg', '5ml', '10ml', '1 tablet', '2 tablets']
const FREQUENT_TESTS = ['CBC', 'Sugar', 'Lipid Profile', 'HbA1c', 'X-Ray Chest', 'ECG']
...
export default function Consultation() {
  ...
  const [vitalTemp, setVitalTemp] = useState({ bp_s: '', bp_d: '', temp: '', wt: '', ht: '', sugar: '', pr: '', spo2: '' })
  ...
  useEffect(() => {
    ...
    if (visit.status === 'vitals_done') {
      visitService.update(visitId, { status: 'in_consultation' }).then(() => {
  ...
  const handleSaveTemplate = async () => {
    ...
    try {
      const complaintsForTemplate = visitComplaints.map(c => c.custom_complaint || (c.complaint?.name || ''))
      const diagnosisForTemplate = visitDiagnosis.map(d => d.custom_diagnosis || (d.diagnosis?.name || '')).join(', ')
      const adviceForTemplate = visitAdvice.map(id => {
  ...
  const handleLoadTemplate = async (template) => {
    if (!confirm(`Load template "${template.name}"? This will overwrite current entries.`)) return
    
    try {
      // Load complaints from template (legacy JSON format)
      const templateComplaints = typeof template.chief_complaints === 'string' 
      ...
      for (const name of templateComplaints) {
        if (!name) continue
        const existing = complaintsMaster.find(c => c.name.toLowerCase() === name.toLowerCase())
        if (existing) {
          await visitRelationsService.addComplaint(visitId, existing.id, null).catch(() => {})
        } else {
          await visitRelationsService.addComplaint(visitId, null, name).catch(() => {})
        }
      }
      const updatedComplaints = await visitRelationsService.getComplaints(visitId)
      ...
      if (template.diagnosis) {
        const diagnoses = typeof template.diagnosis === 'string' ? template.diagnosis.split(',').map(d => d.trim()) : [template.diagnosis]
        for (const diag of diagnoses) {
          if (!diag) continue
          const existing = diagnosisMaster.find(d => d.name.toLowerCase() === diag.toLowerCase())
          if (existing) {
            await visitRelationsService.addDiagnosis(visitId, existing.id, null).catch(() => {})
          } else {
            await visitRelationsService.addDiagnosis(visitId, null, diag).catch(() => {})
          }
        }
      ...
      // Load medicines
      const templateDrugs = typeof template.drugs === 'string' ? JSON.parse(template.drugs || '[]') : (template.drugs || [])
      setMedicines(templateDrugs)
      
      // Load advice
      if (template.advice) {
        const advices = typeof template.advice === 'string' ? template.advice.split(',').map(a => a.trim()) : [template.advice]
        const adviceIds = []
        for (const adv of advices) {
          if (!adv) continue
          const existing = adviceMaster.find(a => a.name.toLowerCase() === adv.toLowerCase())
          if (existing) {
  ...
  const loadData = async () => {
    try {
      const [visitData, vitalsData, complaintsData, diagnosisData, paymentData] = await Promise.all([
      ...
      if (visitData.follow_up_date) {
        const loadedDate = new Date(visitData.follow_up_date).toISOString().split('T')[0]
        ...
        else if (diffDays >= 28 && diffDays <= 31) setSelectedFollowUpOption('1month')
  ...
  const handleAddComplaint = async (e) => {
    ...
    try {
      ...
      try {
        ...
        const errorMsg = masterErr.response?.data?.detail || masterErr.message || 'Failed to create complaint in master'
      ...
      try {
        ...
        const errorMsg = visitErr.response?.data?.detail || visitErr.message || 'Failed to add complaint to visit'
      ...
      const errorMessage = err.message || err.response?.data?.detail || 'Unknown error occurred'
  ...
  const handleAddDiagnosis = async (e) => {
    ...
    try {
      ...
      try {
        ...
        console.error('Master service error:', masterErr)
        const errorMsg = masterErr.response?.data?.detail || masterErr.message || 'Failed to create diagnosis in master'
        throw new Error(`Master: ${errorMsg}`)
      }
      ...
      try {
        ...
        console.error('Visit relations error:', visitErr)
        const errorMsg = visitErr.response?.data?.detail || visitErr.message || 'Failed to add diagnosis to visit'
        throw new Error(`Visit: ${errorMsg}`)
      }
  ...
  const handleAddHistory = async (type) => {
    ...
    try {
      await patientHistoryService.create(visit.patient_id, type, newHistoryValue.trim())
  ...
  const handleAddTest = async () => {
    ...
    try {
      ...
      if (orderedTests.find(t => t.test_name.toLowerCase() === inputValue.toLowerCase())) {
        setNewTestName('')
        return
      }

      // Check if exists in master
      const existing = labTestsMaster.find(t => t.name.toLowerCase() === inputValue.toLowerCase() && t.test_type === 'Lab')
      if (!existing) {
        ...
        try {
          ...
          const errorMsg = err.response?.data?.detail || err.message || 'Failed to create test in master'
          throw new Error(`Master: ${errorMsg}`)
        }
      }
      
      // Add to ordered tests
      setOrderedTests([...orderedTests, { test_name: inputValue, test_type: 'Lab', status: 'ordered' }])
      setNewTestName('')
    } catch (err) {
  ...
  const handleVitalBlur = (which) => {
    if (which === 'bp') {
      const s = parseInt(vitalTemp.bp_s, 10); const d = parseInt(vitalTemp.bp_d, 10)
      if (!isNaN(s) || !isNaN(d)) saveVital({ bp_systolic: isNaN(s) ? null : s, bp_diastolic: isNaN(d) ? null : d })
    } else if (which === 'temp') {
  ...
  const calculateBMI = () => {
    if (vitals?.weight && vitals?.height_cm) {
      const bmi = parseFloat(vitals.weight) / Math.pow(parseFloat(vitals.height_cm) / 100, 2)
  ...
  const handleSave = async (statusToSet = 'completed') => {
    ...
    try {
      ...
      const adviceText = visitAdvice.map(id => {
        const a = adviceMaster.find(ad => ad.id === id)
        return a ? a.name : ''
      }).filter(Boolean).join(', ')
      ...
      if (payment) {
        ...
        await visitRelationsService.createPayment(visitId, docFee, 0, 'pending', null)
      }
  ...
  if (showPrescription) {
    return (
      <PrescriptionView
        ...
        vitals={vitals}
        medicines={medicines}
        chiefComplaints={visitComplaints.map(c => c.custom_complaint || (c.complaint?.name || ''))}
        orderedTests={orderedTests}
        diagnosis={visitDiagnosis.map(d => d.custom_diagnosis || (d.diagnosis?.name || '')).join(', ')}
        advice={visitAdvice.map(id => {
          const a = adviceMaster.find(ad => ad.id === id)
          return a ? a.name : ''
        }).filter(Boolean).join(', ')}
    ...
    <div className="consultation-layout">
      ...
      <div className="patient-sticky-header">
        <div className="header-top">
          <button onClick={() => navigate('/doctor/queue')} className="btn-icon-back">←</button>
          <div className="patient-main-info">
            <h2>{visit?.patient_name}</h2>
            <span className="patient-meta">
              {visit?.patient_age != null && `${visit.patient_age}Y`}
              {visit?.patient_age_months != null && visit.patient_age_months > 0 && ` ${visit.patient_age_months}M`}
            ...
            <div className="header-history-buttons">
              ...
              >
                ...
                {allergyHistory.filter(h => h.is_active).length > 0 && (
                  <span className="history-count-badge">{allergyHistory.filter(h => h.is_active).length}</span>
                )}
              </button>
              <button
                type="button"
              ...
              >
                ...
                {familyHistory.filter(h => h.is_active).length > 0 && (
                  <span className="history-count-badge">{familyHistory.filter(h => h.is_active).length}</span>
                )}
              </button>
              <button
                type="button"
              ...
              >
                ...
                {surgicalHistory.filter(h => h.is_active).length > 0 && (
                  <span className="history-count-badge">{surgicalHistory.filter(h => h.is_active).length}</span>
                )}
              </button>
              <button
                type="button"
              ...
              >
                ...
                {pastHistory.filter(h => h.is_active).length > 0 && (
                  <span className="history-count-badge">{pastHistory.filter(h => h.is_active).length}</span>
                )}
              </button>
              <button
                type="button"
              ...
              >
                ...
                {historyData?.history?.length > 0 && (
                  <span className="history-count-badge">{historyData.history.length}</span>
                )}
              </button>
            </div>
          </div>
        ...
        {vitals && (
          <div className="vitals-strip-single">
            {/* Single line: Height, Weight, BMI, Sugar, BP, Temp, PR, SpO2 */}
            <div className="vital-pill-compact" onClick={() => { setEditingVital('ht'); setVitalTemp(t => ({ ...t, ht: String(vitals.height_cm ?? '') })) }}>
              <span className="vital-icon">📏</span>
              <span className="label-compact">HT</span>
              {editingVital === 'ht' ? (
                <input className="value-inline-compact" type="number" step="0.1" value={vitalTemp.ht} onChange={e => setVitalTemp(t => ({ ...t, ht: e.target.value }))} onBlur={() => handleVitalBlur('ht')} onKeyDown={e => e.key === 'Enter' && handleVitalBlur('ht')} onClick={e => e.stopPropagation()} placeholder="cm" />
              ) : (
                <span className="value-compact">{vitals.height_cm != null ? `${vitals.height_cm}cm` : '—'}</span>
              )}
            </div>
            <div className="vital-pill-compact" onClick={() => { setEditingVital('wt'); setVitalTemp(t => ({ ...t, wt: String(vitals.weight ?? '') })) }}>
              <span className="vital-icon">⚖️</span>
              <span className="label-compact">WT</span>
              {editingVital === 'wt' ? (
                <input className="value-inline-compact" type="number" step="0.1" value={vitalTemp.wt} onChange={e => setVitalTemp(t => ({ ...t, wt: e.target.value }))} onBlur={() => handleVitalBlur('wt')} onKeyDown={e => e.key === 'Enter' && handleVitalBlur('wt')} onClick={e => e.stopPropagation()} placeholder="kg" />
              ) : (
                <span className="value-compact">{vitals.weight != null ? `${vitals.weight}kg` : '—'}</span>
              )}
            </div>
            ...
            <div className="vital-pill-compact" onClick={() => { setEditingVital('sugar'); setVitalTemp(t => ({ ...t, sugar: String(vitals.sugar ?? '') })) }}>
              <span className="vital-icon">🍬</span>
              <span className="label-compact">SUGAR</span>
              {editingVital === 'sugar' ? (
                <input className="value-inline-compact" type="number" step="0.1" value={vitalTemp.sugar} onChange={e => setVitalTemp(t => ({ ...t, sugar: e.target.value }))} onBlur={() => handleVitalBlur('sugar')} onKeyDown={e => e.key === 'Enter' && handleVitalBlur('sugar')} onClick={e => e.stopPropagation()} placeholder="mg/dL" />
              ) : (
                <span className="value-compact">{vitals.sugar != null ? `${vitals.sugar}` : '—'}</span>
              )}
            </div>
            <div className="vital-pill-compact" onClick={() => { setEditingVital('bp'); setVitalTemp(t => ({ ...t, bp_s: String(vitals.bp_systolic ?? ''), bp_d: String(vitals.bp_diastolic ?? '') })) }}>
              <span className="vital-icon">💓</span>
              <span className="label-compact">BP</span>
              {editingVital === 'bp' ? (
                <span className="value-edit-compact" onClick={e => e.stopPropagation()}>
                  <input type="number" value={vitalTemp.bp_s} onChange={e => setVitalTemp(t => ({ ...t, bp_s: e.target.value }))} onBlur={() => handleVitalBlur('bp')} onKeyDown={e => e.key === 'Enter' && handleVitalBlur('bp')} placeholder="S" />
                  <span>/</span>
                  <input type="number" value={vitalTemp.bp_d} onChange={e => setVitalTemp(t => ({ ...t, bp_d: e.target.value }))} onBlur={() => handleVitalBlur('bp')} onKeyDown={e => e.key === 'Enter' && handleVitalBlur('bp')} placeholder="D" />
                </span>
              ) : (
                <span className="value-compact">{vitals.bp_systolic ?? '—'}/{vitals.bp_diastolic ?? '—'}</span>
              )}
            </div>
            <div className="vital-pill-compact" onClick={() => { setEditingVital('temp'); setVitalTemp(t => ({ ...t, temp: String(vitals.temperature ?? '') })) }}>
              <span className="vital-icon">🌡️</span>
              <span className="label-compact">TEMP</span>
              {editingVital === 'temp' ? (
                <input className="value-inline-compact" type="number" step="0.1" value={vitalTemp.temp} onChange={e => setVitalTemp(t => ({ ...t, temp: e.target.value }))} onBlur={() => handleVitalBlur('temp')} onKeyDown={e => e.key === 'Enter' && handleVitalBlur('temp')} onClick={e => e.stopPropagation()} placeholder="°C" />
              ) : (
                <span className="value-compact">{vitals.temperature != null ? `${vitals.temperature}°C` : '—'}</span>
              )}
            </div>
            <div className="vital-pill-compact" onClick={() => { setEditingVital('pr'); setVitalTemp(t => ({ ...t, pr: String(vitals.pr ?? '') })) }}>
              <span className="vital-icon">💗</span>
              <span className="label-compact">PR</span>
              {editingVital === 'pr' ? (
                <input className="value-inline-compact" type="number" value={vitalTemp.pr} onChange={e => setVitalTemp(t => ({ ...t, pr: e.target.value }))} onBlur={() => handleVitalBlur('pr')} onKeyDown={e => e.key === 'Enter' && handleVitalBlur('pr')} onClick={e => e.stopPropagation()} placeholder="bpm" />
              ) : (
                <span className="value-compact">{vitals.pr != null ? `${vitals.pr}` : '—'}</span>
              )}
            </div>
            <div className="vital-pill-compact" onClick={() => { setEditingVital('spo2'); setVitalTemp(t => ({ ...t, spo2: String(vitals.spo2 ?? '') })) }}>
              <span className="vital-icon">🫁</span>
              <span className="label-compact">SpO₂</span>
              {editingVital === 'spo2' ? (
                <input className="value-inline-compact" type="number" value={vitalTemp.spo2} onChange={e => setVitalTemp(t => ({ ...t, spo2: e.target.value }))} onBlur={() => handleVitalBlur('spo2')} onKeyDown={e => e.key === 'Enter' && handleVitalBlur('spo2')} onClick={e => e.stopPropagation()} placeholder="%" />
              ) : (
        ...
        {historyDropdown && historyDropdown !== 'visit' && (
          <>
            <div className="history-panel-overlay" onClick={() => setHistoryDropdown(null)}></div>
            <div className="history-floating-panel">
              <div className="history-panel-header">
                <h4>
                  {historyDropdown === 'allergy' ? '⚠️ Allergy History' : 
                   historyDropdown === 'family' ? '👨‍👩‍👧‍👦 Family History' : 
                   historyDropdown === 'surgical' ? '🏥 Surgical History' : 
                   '📋 Past History'}
                </h4>
                <button type="button" onClick={() => setHistoryDropdown(null)} className="btn-close-panel">✕</button>
              </div>
              <div className="history-panel-body">
                ...
                <div className="history-checkbox-list">
                  {(historyDropdown === 'allergy' ? allergyHistory : historyDropdown === 'family' ? familyHistory : historyDropdown === 'surgical' ? surgicalHistory : pastHistory).map((item) => (
                    <label key={item.id} className="history-checkbox-item-modern">
                      <input
                        type="checkbox"
                        checked={item.is_active}
                        onChange={(e) => handleToggleHistory(item.id, historyDropdown, e.target.checked)}
                      />
                      <span className="history-checkbox-label">{item.value}</span>
                    </label>
                  ))}
                  {((historyDropdown === 'allergy' ? allergyHistory : historyDropdown === 'family' ? familyHistory : historyDropdown === 'surgical' ? surgicalHistory : pastHistory).length === 0) && (
                    <div className="history-empty-state">
                      No {historyDropdown === 'allergy' ? 'allergies' : historyDropdown === 'family' ? 'family history' : historyDropdown === 'surgical' ? 'surgical history' : 'past history'} recorded yet.
                    </div>
                  )}
                </div>
              </div>
      ...
      <div className="consultation-content">
        <div className="template-bar consult-card">
          <div className="template-info">
            <span className="icon">📋</span>
            <h3>Consultation Templates</h3>
          </div>
        ...
        <div className="consultation-sections">
              ...
              <h3>Chief Complaints</h3>
                  ...
                  <div className="chips-grid chips-above-input">
                    {visitComplaints.map((c) => (
                      <span key={c.id} className="complaint-chip active">
                        ...
                        <button type="button" onClick={() => handleRemoveComplaint(c.id)} className="chip-remove">✕</button>
                      </span>
                    ))}
                  </div>
                )}
                ...
                {complaintsMaster.length > 0 && (
                  <div className="master-chips-section">
                    ...
                    <div className="chips-grid master-chips-grid">
                      {complaintsMaster
                        .filter(c => !visitComplaints.find(vc => vc.complaint_id === c.id))
                        .map((c) => (
                          <button
                            key={c.id}
                            type="button"
                            ...
                            onClick={async () => {
                              try {
                                await visitRelationsService.addComplaint(visitId, c.id, null)
                                const updated = await visitRelationsService.getComplaints(visitId)
                                setVisitComplaints(updated)
                              } catch (err) {
                ...
                <div className="complaint-input-group">
                  ...
                  <datalist id="complaint-suggestions">
                    {complaintsMaster.map(c => <option key={c.id} value={c.name} />)}
                  </datalist>
                  {complaintInput.trim() && (
                    <button type="button" onClick={(e) => handleAddComplaint(e)} className="btn-add-complaint" title="Add new to master & visit">+</button>
                  )}
                  <button type="button" onClick={() => setShowComplaintModal(true)} className="btn-show-all" title="Show all complaints">📋</button>
                </div>
              </section>
              ...
              <section className="consult-card compact-card single-row-section">
                  ...
                  <div className="chips-grid chips-above-input">
                    {visitDiagnosis.map((d) => (
                      <span key={d.id} className="complaint-chip active">
                        ...
                        <button type="button" onClick={() => handleRemoveDiagnosis(d.id)} className="chip-remove">✕</button>
                      </span>
                    ))}
                  </div>
                )}
                ...
                {diagnosisMaster.length > 0 && (
                  <div className="master-chips-section">
                    ...
                    <div className="chips-grid master-chips-grid">
                      {diagnosisMaster
                        .filter(d => !visitDiagnosis.find(vd => vd.diagnosis_id === d.id))
                        .map((d) => (
                          <button
                            key={d.id}
                            type="button"
                            ...
                            onClick={async () => {
                              try {
                                await visitRelationsService.addDiagnosis(visitId, d.id, null)
                                const updated = await visitRelationsService.getDiagnosis(visitId)
                                setVisitDiagnosis(updated)
                              } catch (err) {
                ...
                <div className="complaint-input-group">
                  ...
                  <datalist id="diagnosis-suggestions">
                    {diagnosisMaster.map(d => <option key={d.id} value={d.name} />)}
                  </datalist>
                  {diagnosisInput.trim() && (
                    <button type="button" onClick={(e) => handleAddDiagnosis(e)} className="btn-add-complaint" title="Add new to master & visit">+</button>
                  )}
                  <button type="button" onClick={() => setShowDiagnosisModal(true)} className="btn-show-all" title="Show all diagnosis">📋</button>
                </div>
              </section>
            </div>
          ...
          <div className="consultation-row">
            ...
            <section className="consult-card compact-card single-row-section">
                  ...
                  {visitAdvice.map((id) => {
                    ...
                    return (
                      <span key={id} className="complaint-chip active">
                        {a.name}
                        <button type="button" onClick={() => setVisitAdvice(visitAdvice.filter(aid => aid !== id))} className="chip-remove">✕</button>
                      </span>
                    )
                  })}
                </div>
              ...
              <div className="complaint-input-group">
                ...
                <datalist id="advice-suggestions">
                  {adviceMaster.map(a => <option key={a.id} value={a.name} />)}
                </datalist>
                {adviceInput.trim() && (
                  <button type="button" onClick={(e) => handleAddAdvice(e)} className="btn-add-complaint" title="Add new to master & visit">+</button>
                )}
                <button type="button" onClick={() => setShowAdviceModal(true)} className="btn-show-all" title="Show all advice">📋</button>
              </div>
            </section>
            ...
            <section className="consult-card compact-card single-row-section">
              <h3>Investigations / Tests</h3>
                  ...
                  <div className="chips-grid chips-above-input">
                    {orderedTests.map((test, i) => (
                      <span key={i} className="complaint-chip active">
                        {test.test_name}
                        <button type="button" onClick={() => setOrderedTests(orderedTests.filter((_, idx) => idx !== i))} className="chip-remove">✕</button>
                      </span>
                    ))}
                  </div>
                )}
                  ...
                  <div className="master-chips-section">
                    ...
                    <div className="chips-grid master-chips-grid">
                      {labTestsMaster
                        ...
                        .filter(t => !orderedTests.find(ot => ot.test_name === t.name))
                        .map((t) => (
                          <button
                            key={t.id}
                            type="button"
                            className="complaint-chip clickable"
                            onClick={() => {
                              setOrderedTests([...orderedTests, { test_name: t.name, test_type: 'Lab', status: 'ordered' }])
                            }}
                            title="Click to add"
                          >
                            {t.name}
                ...
                {FREQUENT_TESTS.filter(t => ['CBC', 'Sugar', 'Lipid Profile', 'HbA1c'].includes(t))
                  .filter(test => !orderedTests.find(t => t.test_name === test) && !labTestsMaster.find(t => t.name === test))
                  .length > 0 && (
                    ...
                    {FREQUENT_TESTS.filter(t => ['CBC', 'Sugar', 'Lipid Profile', 'HbA1c'].includes(t))
                      ...
                      .map(test => (
                        <button
                          ...
                          onClick={() => {
                            setOrderedTests([...orderedTests, { test_name: test, test_type: 'Lab', status: 'ordered' }])
                          }}
                        >
                          {test}
                        </button>
                ...
                <div className="test-entry-group">
                  ...
                  <datalist id="test-suggestions">
                    {labTestsMaster.filter(t => t.test_type === 'Lab').map(t => <option key={t.id} value={t.name} />)}
                  </datalist>
                  {newTestName.trim() && (
                    <button
                      type="button"
                  ...
                  <tbody>
                    {medicines.map((med, i) => (
                      <tr key={i}>
                        ...
                        <td><span className="qty-pill">{med.quantity}</span></td>
                        <td>{med.instructions || '—'}</td>
                        <td className="text-right med-actions">
                          <button type="button" onClick={() => setMedicines(medicines.filter((_, idx) => idx !== i))} className="btn-table-delete" title="Delete">🗑</button>
                        </td>
                      </tr>
                    ))}
                  </tbody>
                  ...
                  <div className="fee-input-wrapper">
                    ...
                    <input
                      ...
                      onChange={(e) => {
                        ...
                        if (payment) {
                          visitRelationsService.updatePayment(visitId, { doctor_fee: df, lab_fee: 0, total: df }).then(p => setPayment(p))
                        }
                      }}
                      placeholder="0.00"
          ...
          <div className="consult-actions-bar">
            <button onClick={handleMarkPending} className="btn-secondary" disabled={saving}>
              Mark Pending
            </button>
            <button onClick={() => handleSave('completed')} className="btn-primary" disabled={saving}>
      ...
      {showTemplateModal && (
        <div className="modal-overlay">
          <div className="modal-card">
            ...
            <p>This will save current complaints, diagnosis, medications, and advice.</p>
            <div className="form-group">
            ...
            <div className="modal-footer">
              <button onClick={() => setShowTemplateModal(false)} className="btn-back">Cancel</button>
        ...
        <div className="modal-overlay" onClick={() => setShowComplaintModal(false)}>
          <div className="master-modal" onClick={(e) => e.stopPropagation()}>
            <div className="master-modal-header">
              ...
              <button type="button" onClick={() => setShowComplaintModal(false)} className="btn-close">✕</button>
            </div>
            <div className="master-modal-body">
              {complaintsMaster.map((c) => (
                <button
                  key={c.id}
                  type="button"
                  className={`master-item-btn ${visitComplaints.find(vc => vc.complaint_id === c.id) ? 'selected' : ''}`}
                  onClick={async () => {
                    const existing = visitComplaints.find(vc => vc.complaint_id === c.id)
                    if (existing) {
      ...
      {showDiagnosisModal && (
        <div className="modal-overlay" onClick={() => setShowDiagnosisModal(false)}>
          <div className="master-modal" onClick={(e) => e.stopPropagation()}>
            <div className="master-modal-header">
              <h3>All Diagnosis</h3>
              <button type="button" onClick={() => setShowDiagnosisModal(false)} className="btn-close">✕</button>
            </div>
            <div className="master-modal-body">
              {diagnosisMaster.map((d) => (
                <button
                  key={d.id}
                  type="button"
                  className={`master-item-btn ${visitDiagnosis.find(vd => vd.diagnosis_id === d.id) ? 'selected' : ''}`}
                  onClick={async () => {
                    const existing = visitDiagnosis.find(vd => vd.diagnosis_id === d.id)
                    if (existing) {
      ...
      {showAdviceModal && (
        <div className="modal-overlay" onClick={() => setShowAdviceModal(false)}>
          <div className="master-modal" onClick={(e) => e.stopPropagation()}>
            <div className="master-modal-header">
              <h3>All Advice</h3>
              <button type="button" onClick={() => setShowAdviceModal(false)} className="btn-close">✕</button>
            </div>
            <div className="master-modal-body">
              {adviceMaster.map((a) => (
                <button
                  key={a.id}
                  type="button"
                  className={`master-item-btn ${visitAdvice.includes(a.id) ? 'selected' : ''}`}
                  onClick={() => {
      ...
      {historyDropdown === 'visit' && (
        <>
          <div className="history-panel-overlay" onClick={() => { setHistoryDropdown(null); setShowHistory(false); }}></div>
          <div className="history-floating-panel history-visit-panel">
            <div className="history-panel-header">
              <h4>📅 Visit History — {historyData?.patient?.name || visit?.patient_name || 'Patient'}</h4>
              <button type="button" onClick={() => { setHistoryDropdown(null); setShowHistory(false); }} className="btn-close-panel">✕</button>
            </div>
            <div className="history-panel-body">
              {historyLoading && <div className="history-loading-state">Loading visit history...</div>}
              {!historyLoading && historyData?.history?.length === 0 && (
                <div className="history-empty-state">No past visits recorded.</div>
              )}
              {!historyLoading && (historyData?.history || []).map((h, i) => (
                <div key={i} className="visit-history-item">
                  <div className="visit-history-header">
                    <div className="visit-number-badge">{h.visit?.visit_number || 'N/A'}</div>
                    <div className="visit-date">
                      {h.visit?.created_at ? new Date(h.visit.created_at).toLocaleDateString('en-US', { 
                        weekday: 'short', 
                        year: 'numeric', 
                        month: 'short', 
                        day: 'numeric' 
                  ...
                  {h.vitals && (
                    <div className="visit-vitals-grid">
                      {h.vitals.height_cm != null && <div className="visit-vital"><span className="vital-label">HT:</span> {h.vitals.height_cm}cm</div>}
                      {h.vitals.weight != null && <div className="visit-vital"><span className="vital-label">WT:</span> {h.vitals.weight}kg</div>}
                      {h.vitals.bp_systolic != null && <div className="visit-vital"><span className="vital-label">BP:</span> {h.vitals.bp_systolic}/{h.vitals.bp_diastolic}</div>}
                      {h.vitals.temperature != null && <div className="visit-vital"><span className="vital-label">TEMP:</span> {h.vitals.temperature}°C</div>}
                      {h.vitals.pr != null && <div className="visit-vital"><span className="vital-label">PR:</span> {h.vitals.pr}bpm</div>}
                      {h.vitals.spo2 != null && <div className="visit-vital"><span className="vital-label">SpO₂:</span> {h.vitals.spo2}%</div>}
                      {h.vitals.sugar != null && <div className="visit-vital"><span className="vital-label">SUGAR:</span> {h.vitals.sugar}mg/dL</div>}
                    </div>
                  )}
                  {h.visit?.diagnosis && (
                    ...
                    <div className="visit-medications">
                      ...
                      <div className="visit-drugs-list">
                        {h.drugs.slice(0, 5).map((d, j) => (
                          <span key={j} className="visit-drug-chip">{d.drug_name} {d.dosage} {d.frequency}</span>
                        ))}
                        {h.drugs.length > 5 && <span className="visit-drug-chip">+{h.drugs.length - 5} more</span>}
                      </div>
                    </div>
                  )}
                </div>

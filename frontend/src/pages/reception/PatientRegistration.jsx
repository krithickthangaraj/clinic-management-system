...
export default function PatientRegistration() {
  ...
  const [reg, setReg] = useState({
    name: '', guardian_name: '', phone: '', age_years: '', age_months: '0',
    gender: 'male', address: ''
  })
  const [vitals, setVitals] = useState({
    temp_f: '', bp_systolic: '', bp_diastolic: '', pr: '', spo2: '', sugar: '',
    height_cm: '', weight_kg: ''
  })
  ...
  const hasAnyVitals = () =>
    [vitals.temp_f, vitals.bp_systolic, vitals.bp_diastolic, vitals.pr, vitals.spo2, vitals.sugar, vitals.height_cm, vitals.weight_kg]
      .some(v => v !== '' && v != null)

  const buildVitalsPayload = (vid) => {
    const tempC = vitals.temp_f ? (parseFloat(vitals.temp_f) - 32) * 5 / 9 : null
    return {
      visit_id: vid,
      temperature: tempC,
  ...
  const resetForNext = () => {
    ...
    setReg({ name: '', guardian_name: '', phone: '', age_years: '', age_months: '0', gender: 'male', address: '' })
    setVitals({ temp_f: '', bp_systolic: '', bp_diastolic: '', pr: '', spo2: '', sugar: '', height_cm: '', weight_kg: '' })
    setError('')
    fetchToday()
  }
  ...
  useEffect(() => {
    if (!selectedPatient) { setPatientDetails(null); return }
    let cancelled = false
    setLoadingDetails(true)
    setPatientDetails(null)
    const load = async () => {
      try {
        const { patient, history } = await patientService.getHistory(selectedPatient.id)
        if (cancelled) return
        setPatientDetails(patient)
        const latest = history?.[0]?.vitals
        if (latest) {
          const tempF = latest.temperature != null ? String((latest.temperature * 9 / 5 + 32).toFixed(1)) : ''
          setVitals({
            temp_f: tempF,
            bp_systolic: latest.bp_systolic != null ? String(latest.bp_systolic) : '',
            bp_diastolic: latest.bp_diastolic != null ? String(latest.bp_diastolic) : '',
        ...
        setReg({
          ...
          age_months: patient.age_months != null ? String(patient.age_months) : '0',
  ...
  return (
    <div className="reception-page">
        ...
        <aside className="reception-left">
          <div className="card today-card">
            ...
            <div className="today-tabs">
              <button type="button" className={`today-tab ${todayFilter === 'in_queue' ? 'active' : ''}`} onClick={() => setTodayFilter('in_queue')}>
                In Queue <span className="today-count">({countInQueue})</span>
              </button>
              <button type="button" className={`today-tab ${todayFilter === 'pending' ? 'active' : ''}`} onClick={() => setTodayFilter('pending')}>
                Pending <span className="today-count">({countPending})</span>
              </button>
              <button type="button" className={`today-tab ${todayFilter === 'already_seen' ? 'active' : ''}`} onClick={() => setTodayFilter('already_seen')}>
                Already Seen <span className="today-count">({countAlreadySeen})</span>
              </button>
              <button type="button" className={`today-tab ${todayFilter === 'all' ? 'active' : ''}`} onClick={() => setTodayFilter('all')}>
                ...
                <ul className="today-visits">
                  {filteredToday.map((v) => (
                    <li
                      ...
                      onClick={() => {
                        setSelectedPatient({ id: v.patient_id, name: v.patient_name || `#${v.patient_id}` })
                        setVisitId(v.id)
                        setSearchResults([])
                      }}
                    >
                      <div className="today-item-main">
                        <span className="today-name">{v.patient_name || `#${v.patient_id}`}</span>
                        <span className="today-badge" data-status={v.status}>
                          {STATUS_LABELS[v.status] || v.status}
                        </span>
                      </div>
                      <div className="today-item-meta">
                        <span className="today-visit-num">{v.visit_number}</span>
                        <span className="today-time">{formatTime(v.created_at)}</span>
                      </div>
                    </li>
                  ))}
                </ul>
      ...
      <header className="page-header">
        ...
        <button type="button" onClick={() => navigate('/')} className="btn-back">Dashboard</button>
      </header>
          ...
          <ul className="search-results">
            {searchResults.map((p) => (
              <li
                ...
                onClick={() => { setSelectedPatient(p); setSearchResults([]); setSearchQ('') }}
              >
      ...
      {selectedPatient && !loadingDetails && !patientDetails && (
        <section className="card form-grid">
          ...
          <button type="button" onClick={() => { setSelectedPatient(null); setPatientDetails(null); setVisitId(null) }} className="btn-ghost">Choose Another</button>
        </section>
      )}
      ...
      {selectedPatient && patientDetails && !vitalsSaved && (
        <form onSubmit={handleUpdate} className="card form-grid">
          <div className="form-head-row">
            <h3>Edit Patient</h3>
            <button type="button" onClick={() => { setSelectedPatient(null); setPatientDetails(null); setVisitId(null) }} className="btn-ghost btn-sm">Choose Another</button>
          </div>
          <div className="grid-2">
            <div className="field">
              <label>Patient Name *</label>
              <input name="name" value={reg.name} onChange={handleRegChange} required placeholder="Full name" />
            </div>
            <div className="field">
              <label>Guardian Name</label>
              <input name="guardian_name" value={reg.guardian_name} onChange={handleRegChange} placeholder="Guardian" />
            </div>
          </div>
          <div className="grid-3">
            <div className="field">
              <label>Age (Years) *</label>
              <input name="age_years" type="number" min="0" max="150" value={reg.age_years} onChange={handleRegChange} required placeholder="0" />
            </div>
            <div className="field">
              <label>Months</label>
              <input name="age_months" type="number" min="0" max="11" value={reg.age_months} onChange={handleRegChange} placeholder="0" />
            </div>
            <div className="field">
              <label>Gender *</label>
              <select name="gender" value={reg.gender} onChange={handleRegChange}>
          ...
          <div className="field">
            ...
            <input name="phone" type="tel" value={reg.phone} onChange={handleRegChange} required placeholder="10 digits" inputMode="numeric" />
          </div>
          <div className="field">
            <label>Address</label>
            <input name="address" value={reg.address} onChange={handleRegChange} placeholder="Address" />
          </div>
          <h3 className="vitals-subheading">Vitals</h3>
          <div className="vitals-row">
            <div className="field">
              <label>Temp (°F)</label>
              <input name="temp_f" type="number" step="0.1" value={vitals.temp_f} onChange={handleVitalsChange} placeholder="98.6" inputMode="decimal" />
            </div>
            <div className="field">
              <label>BP (mmHg)</label>
              <div className="bp-input">
                <input name="bp_systolic" type="number" value={vitals.bp_systolic} onChange={handleVitalsChange} placeholder="120" />
                <span className="bp-sep">/</span>
                <input name="bp_diastolic" type="number" value={vitals.bp_diastolic} onChange={handleVitalsChange} placeholder="80" />
              </div>
            </div>
            <div className="field">
              <label>PR (bpm)</label>
              <input name="pr" type="number" value={vitals.pr} onChange={handleVitalsChange} placeholder="72" inputMode="numeric" />
            </div>
            <div className="field">
              <label>SpO₂ (%)</label>
              <input name="spo2" type="number" min="0" max="100" value={vitals.spo2} onChange={handleVitalsChange} placeholder="98" inputMode="numeric" />
            </div>
          </div>
          <div className="vitals-row vitals-row-4">
            <div className="field">
              <label>Sugar (mg/dL)</label>
              <input name="sugar" type="number" step="0.1" value={vitals.sugar} onChange={handleVitalsChange} placeholder="100" inputMode="decimal" />
            </div>
            <div className="field">
              <label>Height (cm)</label>
              <input name="height_cm" type="number" step="0.1" value={vitals.height_cm} onChange={handleVitalsChange} placeholder="170" inputMode="decimal" />
            </div>
            <div className="field">
              <label>Weight (kg)</label>
              <input name="weight_kg" type="number" step="0.1" value={vitals.weight_kg} onChange={handleVitalsChange} placeholder="70" inputMode="decimal" />
            </div>
            <div className="field">
      ...
      {vitalsSaved && (
        <section className="card done-card">
          <h3>✓ Done</h3>
          <p>Saved.</p>
          <button type="button" onClick={resetForNext} className="btn-primary">Register Next</button>
        </section>
      )}
        </main>
      </div>
